...
export default function AttackCards() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      fetch(apiUrl('/api/cards/session_summary'))
        .then(res => res.json())
    ...
    const timer = setInterval(() => {
      Object.keys(cardStatusMap).forEach(cardId => {
        ...
        if (lastSt === 'LAUNCHING' || (lastSt && (lastSt.is_running || (!lastSt.detected && lastSt.time_remaining > 0)))) {
          fetch(apiUrl('/api/cards/status/' + cardId))
            .then(res => res.json())
            .then(data => {
  ...
  const handleLaunch = async (card) => {
    ...
    try {
      const res = await fetch(apiUrl('/api/cards/launch/' + card.id), { method: 'POST' });
  ...
  const handleResetAll = async () => {
    if (window.confirm("Reset will stop all attacks and clear session data. Continue?")) {
      await fetch(apiUrl('/api/cards/reset_all'), { method: 'POST' });
      setCardStatusMap({});
      setFeed([]);
  ...
  const handleStopTryAgain = async (cardId) => {
    await fetch(apiUrl('/api/cards/stop/' + cardId), { method: 'POST' });
